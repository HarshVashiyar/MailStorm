...
const AddCompanyModal = ({
  ...
  useEffect(() => {
    if (upd && selectedUsers && selectedUsers.length > 0) {
      // selectedUsers is an array, get the first selected user
      const selectedUserId = Array.isArray(selectedUsers) ? selectedUsers[0] : selectedUsers;

      const userData = users.find(
        (user) => String(user._id) === String(selectedUserId)
      );
            ...
            {!upd && (
              <div className="flex items-center space-x-3">
                ...
                <button
                  ...
                  className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-900 ${isBulkMode ? 'bg-blue-600' : 'bg-gray-600'
                    }`}
                >
                  <span
                    className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${isBulkMode ? 'translate-x-7' : 'translate-x-1'
                      }`}
                  />
                </button>
                <span className={`text-sm font-medium transition-colors ${isBulkMode ? 'text-white' : 'text-gray-400'}`}>
            ...
            <div className="bg-gradient-to-br from-purple-500/20 to-pink-500/20 backdrop-blur-sm border-2 border-purple-400/40 rounded-2xl p-4">
              <div className="flex flex-col items-center justify-center space-y-3">
                ...
                <div className="w-full mt-2">
                  <ExcelUpload refreshUsers={refreshUsers} />
                </div>
              </div>
            </div>
          ) : (
